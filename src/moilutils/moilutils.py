...
class MoilUtils(object):
    ...
    def createMapsMultipleView(cls, numberOfView, typeCamera, listAlpha, listBeta, listZoom, mode=2):
        ...
        for i in range(numberOfView):
            ...
            mapx, mapy = moildev[i].maps_anypoint(listAlpha[i],
                                                    listBeta[i],
                                                    listZoom[i],
                                                    mode)
            mapsX.append(mapx)
    ...
    @classmethod
    def write_camera_type(cls, image_file, typeCamera):
        """
        Read the camera used from metadata image.
    ...
    def saveImage(cls, image, dst_directory, type_camera):
        ...
        cls.write_camera_type(name, type_camera)
        return ss
    ...
    def connectToMoildev(cls, type_camera, parent=None):
        ...
        """
        if type_camera:
            moildev = Moildev.Moildev(file_camera_parameter=cls.__camera_params, camera_type=type_camera)
